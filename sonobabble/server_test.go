...
func TestServe(tester *testing.T) {
	...
	for attempt = 0; attempt <= attemptLimit; attempt++ {
		// Get a response from a get statement to the server, and an error if there is one.
		var (
		...
		/*
			...
			last attempt, fail the test. If neither happen, wait for the timeInterval before the next
			iteration.
		*/
		if response.StatusCode == http.StatusOK {
			break
		} else if response.StatusCode != http.StatusOK && attempt == attemptLimit {
			tester.Fatal(fmt.Errorf("testServe: recieved status code not equal to  200 after the maximum "+
				"%d attempts.", attemptLimit))
		} else {
